...
class ChorganizerBot(commands.Bot):
    ...
    async def setup_hook(self):
        synced = await self.tree.sync(guild=GUILD_OBJ)  # sync slash commands to specific guild
    ...
    async def on_message(self, message):
        ...
        if message.content.lower().startswith('hello') or message.content.lower().startswith('hi'):
            await message.channel.send(f'{message.author.mention}, Dont talk to me rn')
...
async def assign_chores():
    ...
    if now.weekday() != 2:  # Only run on Wednesday (Monday: 0, Tuesday: 1, Wednesday: 2, ...)
        return
    ...
    try:
        ...
        for guild in client.guilds:
            ...
            if not channel:
                print(f"Channel '{TARGET_CHANNEL_NAME}' not found in guild '{guild.name}'")
                continue

            members = sorted([m for m in guild.members if not m.bot], key=lambda m: m.id)
            ...
            for i in range(max(num_members, num_sets)):
                if i < num_members:     # if there are at least enough members as there are chore sets
                    member = members[i]
                    assigned_set = rotated_chore_sets[i % num_sets]     # guaranteed to have at least enough chores for all members due to check in start_cycle
                    assigned_chores = ", ".join(assigned_set)
                    message_lines.append(f"**{member.display_name}** → {assigned_chores}")
                    ...
                    message_lines.append(f"**Unassigned** → {unassigned_chores}")
                    print(f"⚠️ No member available for chore set {i}: {rotated_chore_sets[i]}")
            await channel.send("\n".join(message_lines))
...
@client.tree.command(name="mychore", description="Check your current chore assignment", guild=GUILD_OBJ)
async def my_chore(interaction: discord.Interaction):
    chore = member_assignments.get(interaction.user.id)
    if chore:
        await interaction.response.send_message(f"🧹 Your current chore is: **{chore}**")
    else:
        await interaction.response.send_message("You haven't been assigned a chore yet.")



@client.tree.command(name="add_chores_set", description="Add a set of comma-separated chores to the list of chore sets for me to assign and rotate regularly", guild=GUILD_OBJ)
...
@client.tree.command(name="start_chore_cycle", description="Kickstart the cycle for me to assign chores and rotate them weekly! Make sure you have a full list of chores", guild=GUILD_OBJ)
async def start_cycle(interaction: discord.Interaction):

    # Check if there are enough chore sets for the members in the guild
    for guild in client.guilds:
        members = [m for m in guild.members if not m.bot]  # Get all non-bot members
        num_members = len(members)

        if num_members > len(chore_sets):          # if there are more members than there are chore sets, dont start
            ...
            print(f"⚠️ Cannot start chore cycle: Not enough chore sets for {num_members} members!")
            return  # Exit the function, preventing the cycle from starting
        
    if assign_chores.is_running():
        await interaction.response.send_message("⚠️ Chore cycle is already running.")
    else:
        assign_chores.start()
...
@client.tree.command(name="stop_chore_cycle", description="Stop the ongoing chores rotation. Warning: chores order may not resume if restarted after stopping", guild=GUILD_OBJ)
async def stop_cycle(interaction: discord.Interaction):
    if assign_chores.is_running():
        try:
            ...
            await interaction.response.send_message(f"⚠️ Failed to stop chore cycle: {e}")
    else:
        await interaction.response.send_message("ℹ️ Chore cycle is not currently running.")
